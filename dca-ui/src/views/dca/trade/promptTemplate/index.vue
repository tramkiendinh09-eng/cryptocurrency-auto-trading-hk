<template>
  <div class="app-container">
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span>提示词模板</span>
          <div class="card-header__actions">
            <el-button type="primary" v-hasPermi="['dca:template:add']" @click="handleAdd">新增模板</el-button>
            <el-button plain @click="getList">刷新</el-button>
          </div>
        </div>
      </template>

      <div class="toolbar">
        <el-input v-model="queryParams.name" clearable placeholder="模板名称" style="width: 220px" @keyup.enter="handleQuery" />
        <el-input v-model="queryParams.code" clearable placeholder="模板编码" style="width: 220px" @keyup.enter="handleQuery" />
        <el-select v-model="queryParams.isActive" clearable placeholder="状态" style="width: 160px" @change="handleQuery">
          <el-option label="启用" :value="1" />
          <el-option label="停用" :value="0" />
        </el-select>
      </div>

      <el-table
        v-loading="loading"
        :data="templateList"
        row-key="id"
        highlight-current-row
        empty-text="无提示词模板"
        @current-change="handleCurrentChange"
      >
        <el-table-column prop="name" label="模板名称" min-width="180" />
        <el-table-column prop="code" label="模板编码" min-width="220" show-overflow-tooltip />
        <el-table-column prop="version" label="版本" width="100" />
        <el-table-column label="状态" width="120">
          <template #default="scope">
            <el-tag :type="Number(scope.row.isActive ?? 1) === 1 ? 'success' : 'info'">
              {{ Number(scope.row.isActive ?? 1) === 1 ? '启用' : '停用' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="scope">
            <el-button link type="primary" v-hasPermi="['dca:template:edit']" @click="handleUpdate(scope.row)">修改</el-button>
            <el-button
              link
              :type="Number(scope.row.isActive ?? 1) === 1 ? 'warning' : 'success'"
              v-hasPermi="['dca:template:edit']"
              @click="handleToggle(scope.row)"
            >
              {{ Number(scope.row.isActive ?? 1) === 1 ? '停用' : '启用' }}
            </el-button>
            <el-button link type="danger" v-hasPermi="['dca:template:remove']" @click="handleDelete(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination
        v-show="total > 0"
        :total="total"
        v-model:page="queryParams.pageNum"
        v-model:limit="queryParams.pageSize"
        @pagination="getList"
      />

      <div class="detail-grid">
        <el-card shadow="never" class="detail-card">
          <template #header>
            <div class="detail-card__header">模板详情</div>
          </template>
          <el-descriptions :column="1" border size="small">
            <el-descriptions-item label="模板编码">{{ previewTemplate?.code || '--' }}</el-descriptions-item>
            <el-descriptions-item label="模板名称">{{ previewTemplate?.name || '--' }}</el-descriptions-item>
            <el-descriptions-item label="版本">{{ previewTemplate?.version ?? '--' }}</el-descriptions-item>
            <el-descriptions-item label="状态">{{ Number(previewTemplate?.isActive ?? 1) === 1 ? '启用' : '停用' }}</el-descriptions-item>
            <el-descriptions-item label="默认模板">{{ Number(previewTemplate?.isDefault ?? 0) === 1 ? '是' : '否' }}</el-descriptions-item>
          </el-descriptions>
          <div class="preview-tags">
            <el-tag v-for="item in variableList(previewTemplate?.variablesList || previewTemplate?.variables)" :key="item" size="small" effect="plain">
              {{ item }}
            </el-tag>
          </div>
        </el-card>
        <el-card shadow="never" class="detail-card detail-card--wide">
          <template #header>
            <div class="detail-card__header">模板内容</div>
          </template>
          <pre class="preview-content">{{ previewTemplate?.content || '--' }}</pre>
        </el-card>
      </div>
    </el-card>

    <el-dialog v-model="open" :title="title" width="860px" append-to-body>
      <el-form ref="templateRef" :model="form" :rules="rules" label-width="120px">
        <TradeFormSection title="模板基础" description="维护运行时提示词模板的编码、状态和内容。">
          <el-form-item label="模板名称" prop="name">
            <el-input v-model="form.name" placeholder="Supervisor Primary" />
          </el-form-item>
          <el-form-item label="模板编码" prop="code">
            <el-input v-model="form.code" placeholder="trade.supervisor.v1" />
          </el-form-item>
          <el-form-item label="启用">
            <el-switch v-model="form.isActive" :active-value="1" :inactive-value="0" />
          </el-form-item>
          <el-form-item label="默认">
            <el-switch v-model="form.isDefault" :active-value="1" :inactive-value="0" />
          </el-form-item>
          <el-form-item label="备注">
            <el-input v-model="form.remark" type="textarea" :rows="3" placeholder="模板说明" />
          </el-form-item>
        </TradeFormSection>

        <TradeFormSection title="模板内容" description="变量列表用于标记模板中允许使用的占位符。">
          <el-form-item label="模板内容" prop="content">
            <el-input v-model="form.content" type="textarea" :rows="10" placeholder="You are the trading supervisor for {symbol}" />
          </el-form-item>
          <el-form-item label="变量列表">
            <TradeEditableTags
              v-model="form.variablesList"
              :options="promptTemplateVariableOptions"
              placeholder="输入或选择模板变量"
            />
          </el-form-item>
        </TradeFormSection>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="open = false">取消</el-button>
          <el-button type="primary" :loading="submitting" @click="submitForm">保存</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
</template>

<script>
/**
 * 提示词模板页面工具函数模块
 * 提供模板表单创建、变量解析等辅助函数
 */

function trimValue(value) {
  return String(value ?? '').trim()
}

function parseVariables(value) {
  if (!value || !trimValue(value)) {
    return []
  }
  const normalizedVariables = (list = []) => [...new Set(list.map((item) => trimValue(item)).filter(Boolean))]
  if (Array.isArray(value)) {
    return normalizedVariables(value)
  }
  try {
    const parsed = JSON.parse(value)
    return Array.isArray(parsed) ? normalizedVariables(parsed) : []
  } catch {
    return []
  }
}

export const promptTemplateVariableOptions = [
  'symbol',
  'exchange',
  'mode',
  'event_strength',
  'feature_snapshot_json',
  'event_bundle_json',
  'strategy_context_json',
  'rule_view_json',
  'market_source_status',
  'market_source_context_json',
  'market_context_json',
  'recent_news_context_json',
  'recent_onchain_context_json',
  'market_view_json',
  'news_view_json',
  'onchain_view_json',
  'social_view_json',
  'short_term_memory_json',
  'long_term_memory_json',
  'memory_usage_json',
  'account_equity',
  'daily_pnl',
  'current_position_side',
  'current_position_quantity',
  'current_position_notional',
  'current_position_opened_at',
  'current_time',
  'current_position_holding_minutes',
  'runtime_risk_limits_json',
  'agent_code'
]

export function createPromptTemplateForm(template = {}) {
  return {
    id: template.id,
    name: trimValue(template.name),
    code: trimValue(template.code),
    content: trimValue(template.content),
    variablesList: parseVariables(template.variablesList || template.variables),
    version: template.version ?? null,
    isActive: Number(template.isActive ?? 1),
    isDefault: Number(template.isDefault ?? 0),
    remark: trimValue(template.remark)
  }
}

export function buildPromptTemplatePayload(form = {}) {
  return {
    id: form.id,
    name: trimValue(form.name),
    code: trimValue(form.code),
    content: trimValue(form.content),
    variables: JSON.stringify(parseVariables(form.variablesList)),
    isActive: Number(form.isActive ?? 1),
    isDefault: Number(form.isDefault ?? 0),
    remark: trimValue(form.remark) || null
  }
}

export function validatePromptTemplatePayload(form = {}) {
  const payload = buildPromptTemplatePayload(form)
  const missingFields = []
  if (!payload.name) {
    missingFields.push('模板名称')
  }
  if (!payload.code) {
    missingFields.push('模板编码')
  }
  if (!payload.content) {
    missingFields.push('模板内容')
  }
  if (missingFields.length) {
    throw new Error(`${missingFields.join('、')}不能为空`)
  }
  return payload
}
</script>

<script setup>
import { computed, getCurrentInstance, reactive, ref } from 'vue'

import { addTemplate, delTemplate, listTemplate, toggleTemplate, updateTemplate } from '@/api/dca/template'
import TradeEditableTags from '@/components/trade/TradeEditableTags.vue'
import TradeFormSection from '@/components/trade/TradeFormSection.vue'

const { proxy } = getCurrentInstance()

const loading = ref(false)
const submitting = ref(false)
const open = ref(false)
const title = ref('新增提示词模板')
const templateRef = ref()
const templateList = ref([])
const total = ref(0)
const currentTemplateId = ref(null)
const queryParams = reactive({
  pageNum: 1,
  pageSize: 10,
  name: '',
  code: '',
  isActive: undefined
})
const form = reactive(createPromptTemplateForm())
const rules = {
  name: [{ required: true, message: '模板名称不能为空', trigger: 'blur' }],
  code: [{ required: true, message: '模板编码不能为空', trigger: 'blur' }],
  content: [{ required: true, message: '模板内容不能为空', trigger: 'blur' }]
}

const selectedTemplate = computed(() => templateList.value.find((item) => item.id === currentTemplateId.value) || null)
const previewTemplate = computed(() => (open.value ? form : (selectedTemplate.value || templateList.value[0] || null)))

function variableList(value) {
  return parseVariables(value)
}

function resetForm(template = {}) {
  Object.assign(form, createPromptTemplateForm(template))
  templateRef.value?.clearValidate()
}

function handleCurrentChange(row) {
  currentTemplateId.value = row?.id ?? null
}

function handleQuery() {
  queryParams.pageNum = 1
  getList()
}

async function getList() {
  loading.value = true
  try {
    const response = await listTemplate({
      ...queryParams
    })
    templateList.value = response?.rows || []
    total.value = response?.total || templateList.value.length
    const stillExists = templateList.value.some((item) => item.id === currentTemplateId.value)
    currentTemplateId.value = stillExists ? currentTemplateId.value : (templateList.value[0]?.id ?? null)
  } finally {
    loading.value = false
  }
}

function handleAdd() {
  resetForm()
  title.value = '新增提示词模板'
  open.value = true
}

function handleUpdate(row) {
  resetForm(row)
  title.value = '修改提示词模板'
  open.value = true
}

async function handleToggle(row) {
  const nextStatus = Number(row.isActive ?? 1) === 1 ? 0 : 1
  await toggleTemplate(row.id, nextStatus)
  proxy?.$modal?.msgSuccess?.(nextStatus === 1 ? '提示词模板已启用' : '提示词模板已停用')
  await getList()
}

async function submitForm() {
  await templateRef.value?.validate()
  submitting.value = true
  try {
    const payload = validatePromptTemplatePayload(form)
    if (payload.id) {
      await updateTemplate(payload)
      proxy?.$modal?.msgSuccess?.('提示词模板已更新')
    } else {
      await addTemplate(payload)
      proxy?.$modal?.msgSuccess?.('提示词模板已新增')
    }
    open.value = false
    await getList()
  } catch (error) {
    if (error?.message) {
      proxy?.$modal?.msgError?.(error.message)
    }
    throw error
  } finally {
    submitting.value = false
  }
}

async function handleDelete(row) {
  try {
    await proxy?.$modal?.confirm?.(`确认删除提示词模板“${row.name}”吗？`)
    await delTemplate(row.id)
    proxy?.$modal?.msgSuccess?.('提示词模板已删除')
    await getList()
  } catch {
    // ignore when cancelled
  }
}

getList()
</script>

<style scoped>
.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  gap: 12px;
}

.card-header__actions,
.toolbar {
  display: flex;
  gap: 8px;
  flex-wrap: wrap;
}

.toolbar {
  margin-bottom: 16px;
}

.detail-grid {
  display: grid;
  grid-template-columns: minmax(280px, 360px) minmax(0, 1fr);
  gap: 16px;
  margin-top: 16px;
}

.detail-card {
  min-width: 0;
}

.detail-card--wide {
  min-width: 0;
}

.detail-card__header {
  font-weight: 600;
}

.preview-tags {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
  margin-top: 12px;
}

.preview-content {
  margin: 0;
  padding: 12px;
  min-height: 220px;
  border-radius: 10px;
  background: #f8fafc;
  white-space: pre-wrap;
  word-break: break-word;
  overflow: auto;
}

@media (max-width: 768px) {
  .card-header {
    flex-direction: column;
    align-items: flex-start;
  }

  .detail-grid {
    grid-template-columns: 1fr;
  }
}
</style>
