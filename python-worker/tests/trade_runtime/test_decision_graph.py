...
def test_execution_node_emits_paper_trade_order_callback_in_paper_mode():
    ...
    # 敞口 = 权益 × size_hint × 杠杆。模型没给 leverage_hint，用默认 10 倍，
    # 所以是不带杠杆时 3500 的六倍——杠杆真正参与仓位计算。
    assert callback_client.paper_trade_order_payloads[0]["quoteAmount"] == 35000.0
...
def test_execution_node_passes_limit_order_metadata_without_changing_callbacks():
    ...
    assert captured["order"]["leverage"] == 11
...
def test_graph_executes_paper_order_after_risk_passes():
    ...
    # 同上：默认 10 倍杠杆下敞口是保证金的十倍。
    assert callback_client.order_payloads[0]["quoteAmount"] == 35000.0
